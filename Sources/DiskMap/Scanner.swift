...
struct ScanOptions {
    ...
    ///
    /// One per core. This used to be four per core, on the theory that scanning
    /// is I/O-bound and oversubscribing hides the latency. On a warm cache it is
    /// not I/O-bound at all — it is syscall-bound — and 40 threads on 10 cores
    /// spent their time descheduling each other: a `~/Library` scan ran 2.3s
    /// with 10 workers and 6.1s with 40, for the same work.
